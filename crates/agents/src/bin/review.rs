//! Standalone A2A server for the Review agent.

use panoptes_a2a::AgentServer;
use panoptes_agents::{AgentConfig, ReviewAgent};

const PORT: u16 = 9004;

#[tokio::main]
async fn main() -> anyhow::Result<()> {
    tracing_subscriber::fmt()
        .with_env_filter("info,panoptes=debug")
        .init();

    let (model, llm_table) = load_config();

    let mut agent = ReviewAgent::new(AgentConfig {
        id: "review".into(),
        name: "Review Agent".into(),
        model,
        system_prompt: None,
        mcp_servers: vec![],
        temperature: 0.7,
        max_tokens: 4096,
    });

    // Wire LLM client (no memory for ReviewAgent)
    let llm_config = llm_config_from_toml(&llm_table);
    match panoptes_llm::build_llm_client(&llm_config) {
        Ok(client) => {
            agent = agent.with_llm(client);
            tracing::info!("LLM client initialized ({})", llm_config.model);
        }
        Err(e) => tracing::warn!("Failed to build LLM client, using template fallback: {e}"),
    }

    AgentServer::new(agent, PORT).run().await
}

fn llm_config_from_toml(table: &toml::Value) -> panoptes_llm::LlmConfig {
    table
        .clone()
        .try_into()
        .unwrap_or_else(|_| panoptes_llm::LlmConfig {
            provider: "openai".into(),
            model: "lfm2:24b".into(),
            api_url: Some("http://host.orb.internal:11434".into()),
            api_key: None,
            temperature: None,
            max_tokens: None,
            max_concurrent_requests: 2,
            retry: Default::default(),
        })
}

fn load_config() -> (String, toml::Value) {
    let path = std::env::var("PANOPTES_CONFIG").unwrap_or_else(|_| "config/default.toml".into());
    let text = std::fs::read_to_string(&path).unwrap_or_default();
    let table: toml::Value = text
        .parse()
        .unwrap_or(toml::Value::Table(Default::default()));

    let model = table
        .get("llm")
        .and_then(|v| v.get("model"))
        .and_then(|v| v.as_str())
        .unwrap_or("lfm2:24b")
        .to_string();

    let llm_table = table
        .get("llm")
        .cloned()
        .unwrap_or(toml::Value::Table(Default::default()));

    (model, llm_table)
}
