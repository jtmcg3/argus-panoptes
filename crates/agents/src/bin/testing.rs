//! Standalone A2A server for the Testing agent.

use panoptes_a2a::AgentServer;
use panoptes_agents::{AgentConfig, TestingAgent};
...
async fn main() -> anyhow::Result<()> {
    ...
    let (model, llm_table) = load_config();

    let mut agent = TestingAgent::new(AgentConfig {
        id: "testing".into(),
    ...
    // Wire LLM client (no memory for TestingAgent)
...
fn load_config() -> (String, toml::Value) {
