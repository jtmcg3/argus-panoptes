//! Standalone A2A server for the Planning agent.

use panoptes_a2a::AgentServer;
use panoptes_agents::{AgentConfig, PlanningAgent};
use std::sync::Arc;
...
async fn main() -> anyhow::Result<()> {
    ...
    let (model, llm_table, memory_table) = load_config();

    let mut agent = PlanningAgent::new(AgentConfig {
        id: "planning".into(),
    ...
    // Wire LLM client
    let llm_config = llm_config_from_toml(&llm_table);
    ...
    // Wire memory store
    let mem_config: panoptes_memory::MemoryConfig = memory_table.try_into().unwrap_or_default();
...
fn llm_config_from_toml(table: &toml::Value) -> panoptes_llm::LlmConfig {
    table
        .clone()
        .try_into()
...
fn load_config() -> (String, toml::Value, toml::Value) {
